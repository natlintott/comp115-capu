...
def reverse_str(s): 
    ...
    Returns:
    reverse (string): A reversed version of string s.
    """
...
def remove_duplicates(s):
    ...
    Parameters:
    - s (string): The input string for duplicate characters to be removed.

    Returns:
    - (str): The input string s with all duplicate characters removed 
    """


# Your unit tests
assert remove_duplicates("apple") == "aple"
assert remove_duplicates("AabbCeeDee") == "AabCeD"
assert remove_duplicates("word") == "word"
...
"""
def find_index(s, t):
    """This function returns the index of the first appearance of a character t in string s.""" 
    index = 0
    for i in s:
        if i == t:
            return index
    ...
    Parameters:
    - s (str): The string in which the character t will be searched for
    - t (str): The character that is searched for in string s

    Returns:
    - (int): The index of the character t in string s. If the character 
    is not found in string s, -1 is returned.
    """



# Your unit tests
assert find_index("Apples", 'p') == 1
assert find_index("Mississippi", 's') == 2
assert find_index("Large", 'E') == -1
...
def project_completion_day(day, days_to_completion):
    """This function calculates and returns what day of the week a project will be completed"""
    day = days_week.index(day)
    completed_day = (day + days_to_completion) % 7 
    return (days_week[completed_day])
    """
    E.g., 
    >>> project_completion_day('Monday', 1)
    'Tuesday'

    Parameters:
    - day (str): The starting day of the week of the project
    - days_to_completion (int): The amount of days the project will take to complete

    Returns:
    - (str): The day of the week the project will be completed on.
    """


# Your unit tests
assert project_completion_day('Sunday', 1) ==  'Monday'
assert project_completion_day('Monday', 7) == 'Monday'
assert project_completion_day('Saturday', 14) ==  'Saturday'
assert project_completion_day('Saturday', 20) ==  'Friday'
...
def parse_log_line(line):
    """This function parses a log (line) into its components and returns the result in a tuple"""
    lineaslist = line.split()
    timestamp = lineaslist[0] + " " + lineaslist[1]
    level = lineaslist[2]
    module = lineaslist[3]
    messagelist = lineaslist[4:]
    message = ""
    for word in messagelist:
        message = message + word
        if word != messagelist[-1]:
            message = message + " "
