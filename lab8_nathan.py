...
def remove_duplicates_set(s):
    """This function removes duplicate characters from string s, using a set."""
    seen_set = set()
    res = ""
    for i in s:
    ...
    """
    Parameters:
    - s (string): The string in which duplicate characters will be removed.

    Returns:
    - res (string): The result of removing duplicate characters from string s.
    """

assert remove_duplicates_set("apple") == "aple"
assert remove_duplicates_set("Popsipple") == "Popsile"
assert remove_duplicates_set("123123andGoandGo") == "123andGo"
...
def gem_counting(stones, gems):
    """This function counts the amount of characters from string gems that can be found in string stones."""
    gemlist = set()
    gems_total = 0
    for i in gems:
        if i not in gemlist:
    ...
    """
    Parameters:
    - stones (string): The string that will be searched for characters from string gems.

    - gems (string): The string that string stones will be compared to .

    Returns:
    - gems_total (int): The total amount of characters from string gems that are found in string stones.
    """

assert gem_counting("abDFMdm", "admMQq") == 4
assert gem_counting("abDFMdm", "af") ==  1
assert gem_counting("stones", "gems") ==  3
assert gem_counting("Coal", "Diamond") == 2
...
def students_id(ids):
    """This function counts the amount of unique student ids in list ids."""
    seen_id = set()
    for i in ids: 
        if i not in seen_id:
            seen_id.add(i)
    ...
    """
    Parameters:
    - ids (list): The list of ids that unique ids will be searched for.

    Returns:
    - (int): The total unique ids found in string ids.
    """

assert students_id(['002', '003', '001', '004', '012', '012', '012', '012']) == 5
assert students_id(['001', '001', '001', '111', '101', '011']) == 4 
...
def students_id_occurrences(ids):
    """This function counts the occurences of each student id in list ids, and returns the result as a dictionary."""
    id_dict = {}
    for i in ids:
        if i not in id_dict:
            id_dict[i] = 1
    ...
    """
    Parameters:
    - ids (list): The list of ids that occurences of each id will be searched for.

    Returns:
    - id_dict (dict): The total occurences of each student id found in string ids, represented as a dictionary.
    """

assert students_id_occurrences(['002', '003', '001', '004', '012']) == {'002': 1, '003': 1, '001': 1, '004': 1, '012': 1}
assert students_id_occurrences(['001', '001', '001', '111', '101', '011']) == {'001': 3, '111': 1, '101': 1, '011': 1}
...
def word_frequency(paragraph):
    """This function returns the frequency of each word used in string paragraph and returns the result as a dictionary."""
    words = re.findall((r'\b\w+\b').lower, paragraph)
    word_count = {}
    for i in words:
    ...
    """
    Parameters:
    - paragraph (string): The string in which word frequency will be counted.

    Returns:
    - word_count (dict): The total occurences of each word found in string paragraph, represented as a dictionary.
    """

assert word_frequency("I am alive. I am happy.") == {'I': 2, 'am': 2, 'alive': 1, 'happy': 1}
assert word_frequency("Apple, apple. Apples, apple!") == {'Apple': 1, 'apple': 2, 'Apples': 1}
